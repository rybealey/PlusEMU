    ...
    public void SendObjects(GameClient session)
    {
        ...
        foreach (var user in _roomUserManager.GetUserList().ToList())
        {
            ...
            if (!user.IsBot && !user.IsPet && user.GetClient()?.GetHabbo() != null)
            {
                ...
                session.Send(new RpStatsComposer(user.VirtualId, habbo.RpHealth, habbo.RpHealthMax, habbo.RpEnergy, habbo.RpEnergyMax, (int)Math.Round(habbo.RpAggression), habbo.RpPassiveSeconds > 0 ? 1 : 0));
            }
