...
public static class BankUtility
{
    ...
    /// <summary>
    /// Take money out of checking, or say it could not. No cash, no fee, no
    /// second account - one guarded UPDATE that cannot leave a balance below
    /// zero however many things ask at once.
    ///
    /// For callers that have their OWN fallback when the bank cannot pay: a
    /// shop sale charges the card and reaches for the wallet when it bounces,
    /// and a refusal here is that bounce rather than the end of the matter.
    /// </summary>
    public static bool TryDebitChecking(int userId, string username, long amount, string source, out BankAccount? account)
    {
        account = null;
        ...
        lock (LockFor(userId))
        {
            try
            {
                ...
                if (taken == 0)
                    return false;
                account = Refresh(connection, userId);
                LogMovement(connection, userId, username, BankTransactionKind.PayOut,
                    BankAccountKind.Current, -amount, account?.Current ?? 0, Fit(source));
                return true;
            }
            catch (Exception e)
            {
                Log.Error("TryDebitChecking for {0} failed: {1}", userId, e.Message);
                return false;
            }
        }
    }

    /// <summary>Put money into checking, or say there was no account to put it in.</summary>
    public static bool CreditChecking(int userId, string username, long amount, string source, out BankAccount? account)
    {
        account = null;
        if (userId <= 0 || amount <= 0)
            return false;
        lock (LockFor(userId))
        {
            ...
            {
                using var connection = PlusEnvironment.DatabaseManager.Connection();
                var wasLoaded = Accounts.ContainsKey(userId);
                var given = connection.Execute(
                    "UPDATE `rp_bank_accounts` SET `current_balance` = `current_balance` + @amount " +
                    "WHERE `user_id` = @userId LIMIT 1",
                    new { userId, amount });
                if (given == 0)
                    return false;
                account = Refresh(connection, userId);
                LogMovement(connection, userId, username, BankTransactionKind.PayIn,
                    BankAccountKind.Current, amount, account?.Current ?? 0, Fit(source));
                // Somebody offline must not be left in the cache the online
                // paths write from - the same rule Pay follows.
                if (!wasLoaded)
                    Accounts.TryRemove(userId, out _);
                return true;
            }
            ...
            {
                Log.Error("CreditChecking for {0} failed: {1}", userId, e.Message);
                return false;
            }
        }
    }
