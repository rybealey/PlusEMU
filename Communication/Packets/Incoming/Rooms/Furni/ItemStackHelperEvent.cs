...
/// in hundredths, the same format the rest of the stack-height plumbing uses.
/// Nothing here handled it - 3839 was not in ClientPacketHeader at all, and
/// UpdateMagicTileEvent, which the constant list DOES name, has no handler
/// class either - so the slider only ever moved the sprite on the builder's
/// own screen. Nothing was stored, and the height snapped back the moment
/// anything made the client redraw: a reload, a rotate, a drag.
///
...
/// road `:bh` takes.
/// </summary>
internal class ItemStackHelperEvent : IPacketEvent
{
    ...
    public ItemStackHelperEvent(IDatabase database) => _database = database;
