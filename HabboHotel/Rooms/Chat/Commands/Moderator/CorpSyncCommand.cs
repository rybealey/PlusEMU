...
/// <summary>
/// pixelrp: :corpsync [corp_key] - staff re-broadcast of employment for one
...
internal class CorpSyncCommand : IChatCommand
{
    ...
    public string Parameters => "%corp_key%";
    ...
    public void Execute(GameClient session, Room room, string[] parameters)
    {
        ...
        if (parameters.Any())
        {
            ...
            corpId = connection.QuerySingleOrDefault<int>(
                "SELECT `id` FROM `rp_corporations` WHERE `corp_key` = @key LIMIT 1", new { key = parameters[0].ToLower() });
            if (corpId == 0)
            {
                session.SendWhisper($"No corporation with the key '{parameters[0]}'.");
                return;
            }
        }
