    ...
    private bool ProcessRoom(RoomMovement room, long now)
    {
        ...
        lock (room.MovementLock)
        {
                   ...
                   && drained < MovementSettings.MaxDrainPerRoom)
            {
                ...
                // NOTE, should a pre-commit gate ever be needed here again:
                // it must never simply `break` while leaving the walker at a due
                // tick in the past. ComputeNextDue would return that past tick,
                // the room would be instantly due again, and the scheduler would
                // spin hot on its single thread - starving the whole hotel, not
                // just this room. Defer the blocked walker strictly forward and
                // keep draining the others instead.
